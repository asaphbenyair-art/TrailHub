...
export default function RideshareBoard({ tripId }: { tripId: string }) {
  ...
  if (!allowed) {
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-4 text-center">
        <div className="text-sm font-medium text-gray-900 mb-1">🚗 לוח טרמפים</div>
        <div className="text-xs text-gray-400">זמין לנרשמים ומתעניינים בטיול</div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4">
      ...
      {showForm && (
        <div className="border border-gray-200 rounded-xl p-3 mb-3 flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <input type="number" min="1" value={spots} onChange={(e) => setSpots(e.target.value)} placeholder="מקומות"
              className="w-24 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#1A6B4A]" dir="ltr" />
      ...
      {loading && <div className="text-center py-4 text-gray-400 text-xs">טוען...</div>}
      {!loading && offers.length === 0 && (
        <div className="text-center py-6 text-gray-400 text-xs">אין טרמפים עדיין — היה הראשון לפרסם</div>
      )}
              ...
              <div className="flex items-center gap-2 mt-2">
                {isPoster ? (
                  <button type="button" onClick={() => leave(o.id)}
                    className="text-[11px] text-red-500 border border-red-200 rounded-full px-3 py-1 hover:bg-red-50">בטל טרמפ</button>
                ) : iClaimed ? (
                  <>
