      ...
      <div className="relative w-full max-w-[460px] max-h-[86vh] bg-surface rounded-2xl shadow-2xl flex flex-col overflow-hidden"
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border shrink-0">
          ...
          <div className="flex-1 text-sm text-fg truncate">
            <span className="inline-flex items-center gap-1.5"><MessageCircle size={15} className="text-[#1A6B4A]" /> שאלות ותשובות</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
          ...
          {!loading && shown.length === 0 && (
            <p className="text-xs text-fg-faint text-center py-6">
              {view === "mine" ? "לא שאלת שאלות עדיין." : "אין שאלות ממטיילים אחרים עדיין."}
            </p>
          )}
          {shown.map((q) => (
          ...
          {session ? (
            <div className="flex flex-col gap-2">
              <div className="flex gap-2">
                <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="שאל שאלה…" rows={2}
              ...
              <div className="inline-flex bg-surface-2 rounded-full p-0.5 self-start text-[11px]">
                <button type="button" onClick={() => setIsPrivate(false)}
                  className={`px-2.5 py-1 rounded-full font-medium ${!isPrivate ? "bg-[#1A6B4A] text-white" : "text-fg-muted"}`}>שאלה גלויה</button>
                <button type="button" onClick={() => setIsPrivate(true)}
                  className={`px-2.5 py-1 rounded-full font-medium ${isPrivate ? "bg-[#185FA5] text-white" : "text-fg-muted"}`}>🔒 שאלה פרטית</button>
              </div>
            </div>
          ) : (
