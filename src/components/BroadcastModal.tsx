...
export default function BroadcastModal({
  ...
  async function send() {
    setError(null);
    if (!body.trim()) { setError(tb(isCancellation ? "errReason" : "errEmpty")); return; }
    setSending(true);
    try {
      ...
      <div className="relative w-full max-w-[460px] max-h-[86vh] bg-surface rounded-2xl shadow-2xl flex flex-col overflow-hidden"
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border shrink-0">
          ...
          <div className="flex-1 text-sm text-fg truncate">
            <span className="inline-flex items-center gap-1.5"><Megaphone size={15} className="text-[#1A6B4A]" /> {tb("title")}</span>
        ...
        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
          ...
          {!loading && list.length === 0 && (
            <p className="text-xs text-fg-faint text-center py-6">{tb("empty")}</p>
          )}
          {list.map((b) => (
            <div key={b.id} className="rounded-xl p-3 border"
              ...
              <div className="flex items-center gap-2 mb-1">
                {b.isCancellation && <span className="text-[10px] font-bold text-[#C0392B]">⚠ {tb("cancelTrip")}</span>}
        ...
        <div className="border-t border-border p-3 shrink-0">
          {!composing ? (
            <button type="button" onClick={() => setComposing(true)}
              ...
              <Plus size={15} /> {tb("newMessage")}
            </button>
          ) : (
            <div className="flex flex-col gap-2">
              <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={3}
                placeholder={isCancellation ? tb("cancelPlaceholder") : tb("placeholder")}
              ...
              <label className="flex items-center gap-2 text-xs text-fg-muted cursor-pointer">
                ...
                <span className={isCancellation ? "text-[#C0392B] font-medium" : ""}>{tb("cancelTripCheckbox")}</span>
              </label>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={send} disabled={sending || !body.trim()}
                  ...
                  {sending ? tb("sending") : isCancellation ? tb("sendCancel") : tb("send")}
                </button>
                <button type="button" onClick={() => { setComposing(false); setError(null); setIsCancellation(false); }}
                  className="px-4 py-2 rounded-full text-sm font-medium border border-border text-fg-muted">{tc("cancel")}</button>
              </div>
            </div>
          )}
        </div>
