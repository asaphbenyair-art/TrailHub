  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#f5f5f5]">
      <div className="max-w-[480px] mx-auto px-3 py-3 pb-8">
        ...
        {/* Top bar */}
        <div className="bg-white rounded-xl px-3 py-2.5 mb-2 flex items-center gap-2.5">
          ...
          {session ? (
            ...
            <Link href="/auth/login" className="text-xs text-[#1A6B4A] border border-[#1A6B4A] rounded-full px-2.5 py-1 flex-shrink-0 whitespace-nowrap">
              כניסה
            </Link>
          )}
        </div>
        ...
        <div className="flex bg-gray-100 rounded-full p-1 mb-2 gap-1">
          <button
            type="button"
            onClick={() => setViewMode("list")}
            className={`flex-1 py-1.5 rounded-full text-xs font-medium transition-all ${
              viewMode === "list" ? "bg-white text-[#1A6B4A] shadow-sm" : "text-gray-500"
            }`}
          >
            ☰ רשימה
          </button>
          <button
            ...
            className={`flex-1 py-1.5 rounded-full text-xs font-medium transition-all ${
              viewMode === "calendar" ? "bg-white text-[#1A6B4A] shadow-sm" : "text-gray-500"
            }`}
          >
            📅 יומן
          </button>
        ...
        {/* Calendar view — replaces list when active */}
        {viewMode === "calendar" && (
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 mb-1.5" style={{ scrollbarWidth: "none" }}>
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs whitespace-nowrap border flex-shrink-0 transition-colors ${
              panelOpen || activeCount > 0 ? "bg-[#D6EDE3] border-[#1A6B4A] text-[#0F5038]" : "bg-white border-gray-200 text-gray-600"
            }`}
          >
            ⚙ פילטרים
            {activeCount > 0 && (
              <span className="bg-[#1A6B4A] text-white rounded-full min-w-[16px] h-4 px-1 text-[10px] leading-4 inline-flex items-center justify-center">
                {activeCount}
              </span>
            )}
          </button>
        ...
        <div className="flex items-center justify-between mb-2 px-0.5">
          <span className="text-xs text-gray-500">
            {loading ? "טוען..." : `${trips.length} טיולים נמצאו`}
          </span>
          <select
            value={filters.sort}
              ...
              <div key={trip.id} className="bg-white rounded-2xl overflow-hidden border border-gray-100 cursor-pointer"
                ...
                {myStatus && (
                  <div className={`px-3 py-1.5 flex items-center gap-1.5 text-xs font-medium ${
                    ...
                    myStatus === "WAITLIST"  ? "bg-[#D4E4F0] text-[#185FA5]" :
                ...
                <div className="relative" style={{ height: 160 }}>
                  ...
                  <button type="button" onClick={(e) => e.stopPropagation()}
                    className="absolute top-2.5 left-2.5 bg-black/40 rounded-full w-7 h-7 flex items-center justify-center text-white text-sm">
                    ♡
                  </button>
