...
export default function HikerChatPage() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#f5f5f5] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-1 pb-24">
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id}>
              {showDay && (
                <div className="text-center text-[10px] text-gray-400 my-3">{dayStr}</div>
              )}
      ...
      {/* Input */}
      <div
        className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-3 flex gap-2 items-end"
        dir="rtl"
      >
        <textarea
          value={body}
