    ...
    <div className="flex flex-col gap-2">
          ...
          {materials.map((m, i) => (
            <div key={i} className="bg-surface-2 rounded-lg px-3 py-1.5 flex flex-col gap-1">
              <div className="flex items-center gap-2 text-xs">
                ...
                <button type="button" onClick={() => remove(i)} className="text-fg-faint hover:text-red-400">✕</button>
              </div>
      ...
      <div className="flex gap-2">
        <input type="text" value={linkTitle} onChange={(e) => setLinkTitle(e.target.value)} placeholder="כותרת"
          className="w-28 border border-border rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:border-[#1A6B4A]" />
        <input type="url" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} placeholder="קישור (https://...)" dir="ltr"
          className="flex-1 border border-border rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:border-[#1A6B4A]" />
