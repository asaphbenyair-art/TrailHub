    ...
    <div className="min-h-screen flex items-center justify-center p-4" dir="rtl">
      <div className="w-full max-w-[420px]">
        <div className="text-center mb-8">
          ...
          <p className="text-fg-muted text-sm">בחירת סיסמה חדשה</p>
        </div>

        <div className="bg-surface rounded-2xl border border-border shadow-sm p-6">
          {invalidLink ? (
            <div className="text-center">
              <p className="text-sm text-fg-muted mb-4">הקישור אינו תקין. בקש קישור חדש לאיפוס.</p>
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-fg-muted mb-2">הסיסמה עודכנה בהצלחה!</p>
              <p className="text-xs text-fg-faint">מעביר אותך להתחברות...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              {error && <div className="bg-surface-2 border border-red-200 text-red-700 rounded-lg px-4 py-3 text-sm">{error}</div>}
              <div className="flex flex-col gap-1">
                <label className="text-xs text-fg-muted">סיסמה חדשה</label>
                <input
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-fg-muted">אימות סיסמה</label>
                <input
                  type={show ? "text" : "password"}
                  value={confirm}
              ...
              <button type="button" onClick={() => setShow((v) => !v)} className="text-xs text-fg-faint text-right hover:text-fg-muted">
